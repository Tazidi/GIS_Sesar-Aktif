...
@if(isset($map))
    ...
    [
        @foreach ($maps as $index => $map)
        {
            ...
            "image_path": "{{ $map->image_path ? asset($map->image_path) : '' }}",            "layer_type": "{{ $map->layer_type ?? 'marker' }}",
            ...
            "features": [
                @foreach ($map->features as $feature)
                {
                    ...
                    "image_path": "{{ $feature->image_path ? asset('map_features/' . $feature->image_path) : '' }}",                    "caption": @json($feature->caption ?? ''),
                    "technical_info": @json($feature->technical_info ?? '')
                }@if(!$loop->last),@endif
                @endforeach
            ]
        }@if(!$loop->last),@endif
    ...
    <script>
        ...
        function displayDetailContent(featureData) {
            ...
            if (featureData.dataSource === 'manual') {
                ...
                if (featureData.photo) {
                    content += `<div class="detail-item"><div class="detail-label">Foto</div><div class="detail-value">
                        <img src="${featureData.photo}" alt="Foto" style="max-width: 100%; border-radius: 8px; cursor: pointer;"
                            ...
                            onerror="this.parentElement.innerHTML = '<i>Gagal memuat</i>';"></div></div>`;
                }
                ...
                const techInfo = properties.technical_info; // simpan dulu
                ...
                if (featureData.image_path) {
                    content += `<div class="detail-item"><div class="detail-label">Foto</div><div class="detail-value">
                        <img src="${featureData.image_path}" alt="Foto" style="max-width: 100%; border-radius: 8px; cursor: pointer;"
                            onclick="window.open('${featureData.image_path}', '_blank')"
        ...
        function createPopupContent(feature, mapData) {
            ...
            if (isGeoJSON) {
                ...
                dataForModal = { 
                    ...
                    caption: props.caption || '',   // <--- tambahkan
                ...
                dataForModal = { 
                    ...
                    caption: mapData.caption || ''    // <--- tambahkan
                };
            }
            ...
            return `
                ...
                <button class="btn-detail" data-feature='${encoded}'>Lihat Detail</button>
            `;
        }
                ...
                if (mapData.features && mapData.features.length > 0) {
                    mapData.features.forEach(feature => {
                        ...
                        const geojsonFeature = {
                            "type": "Feature",
                            "geometry": geometry,
                            "properties": { ...(feature.properties || {}), image_path: feature.image_path || null, caption: feature.caption || null, technical_info: feature.technical_info || null }
                        };
                        const layer = L.geoJSON(geojsonFeature, {
                            style: () => style,
                            onEachFeature: function(feature, layer) {
                                feature.image_path = geojsonFeature.properties.image_path;                                feature.caption = geojsonFeature.properties.caption;
